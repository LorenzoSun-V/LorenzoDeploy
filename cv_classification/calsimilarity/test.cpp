...
std::vector<input_search_param> readSearchLibFromDirectory(const std::string& dir_path) {
    ...
    for (const auto& entry : fs::directory_iterator(dir_path)) {
        if (entry.is_regular_file()) {
            ...
            // 提取 feature_id（假设文件名以"ID_名字"格式，用"_"分隔）
            size_t underscore_pos = file_name.find('_');
            if (underscore_pos != std::string::npos) {
                // 提取 ID 部分并转换为整数
                int feature_id = std::stoi(file_name.substr(0, underscore_pos));
                // 读取特征向量
                std::vector<float> feature_value = readFeatureFromFile(file_path);
                // 存储到 search_lib
...
int main() {
    ...
    // 从目录中读取查询库
    std::vector<input_search_param> query_lib = readSearchLibFromDirectory(input_feat_path);
    if (query_lib.empty()) {
        std::cerr << "Failed to read query library." << std::endl;
        return -1;
    }
