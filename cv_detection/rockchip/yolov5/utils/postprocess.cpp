...
static int process(int classnum, uint8_t *input, int *anchor, int grid_h, int grid_w, int stride,
                   std::vector<float> &boxes, std::vector<float> &objProbs, std::vector<int> &classId,
                   float threshold, uint32_t zp, float scale)
{
    int objClassNum = 5+classnum;
    int validCount = 0;
    ...
    for (int a = 0; a < 3; a++)
    {
        for (int i = 0; i < grid_h; i++)
        {
            ...
            {
                uint8_t box_confidence = input[(objClassNum * a + 4) * grid_len + i * grid_w + j];
                if (box_confidence >= thres_u8)
                {
                    int offset = (objClassNum * a) * grid_len + i * grid_w + j;
...
int post_process(int classnum, uint8_t *input0, uint8_t *input1, uint8_t *input2, int model_in_h, int model_in_w,
                 ...
                 detect_result_group_t *group)
{
    ...
    int validCount0 = 0;
    validCount0 = process(classnum, input0, (int *)anchor0, grid_h0, grid_w0,
    ...
    int validCount1 = 0;
    validCount1 = process(classnum, input1, (int *)anchor1, grid_h1, grid_w1,
    ...
    int validCount2 = 0;
    validCount2 = process(classnum, input2, (int *)anchor2, grid_h2, grid_w2,
