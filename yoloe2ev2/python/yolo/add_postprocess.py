...
给yolov7 的onnx拼接几个操作
实现将yolov7 的output: [1, 25200,85]  # 85在我们这里是9
...
boxes input: [1,25200, 1, 4]
scores_input: [1, 25200, 80] # 80是类别个数，我们是4个人类别
...
boxes: [1,25200,1,4]  # x_center,y_center, w,h--> x1,y1,x2,y2

socres: [1,25200,80]  # 80个类别的概率
...
def add_postprocess(input_name, numClasses, output_name):
    ...
    _, num_array, _ = origin_output.outputs[0].shape # yolo shape: [batch_size, num_arry, 5+cls]
    
    starts_wh = gs.Constant("starts_wh",values=np.array([0,0,0],dtype=np.int64))
    ends_wh = gs.Constant("ends_wh",values=np.array([1,num_array,4],dtype=np.int64))
    ...
    ends_object = gs.Constant("ends_object",values=np.array([1,num_array,5],dtype=np.int64))

    starts_conf = gs.Constant("starts_conf",values=np.array([0,0,5],dtype=np.int64))
    ends_conf = gs.Constant("ends_conf",values=np.array([1,num_array, numClasses+5],dtype=np.int64))

    # output
    box_xywh_0 = gs.Variable(name="box_xywh_0",shape=(1,num_array,4),dtype=np.float32)
    object_prob_0 = gs.Variable(name="object_prob_0",shape=(1,num_array,1),dtype=np.float32)
    label_conf_0 = gs.Variable(name='label_conf_0',shape=(1,num_array, numClasses),dtype=np.float32)
    ...
    # identity
    box_xywh = gs.Variable(name="box_xywh",shape=(1,num_array,4),dtype=np.float32)
    object_prob = gs.Variable(name="object_prob",shape=(1,num_array,1),dtype=np.float32)
    label_conf = gs.Variable(name='label_conf',shape=(1,num_array, numClasses),dtype=np.float32)
    ...
    ends_1 = gs.Constant("ends_x",values=np.array([1,num_array,1],dtype=np.int64))
    ...
    ends_2 = gs.Constant("ends_y",values=np.array([1,num_array,2],dtype=np.int64))

    starts_3 = gs.Constant("starts_w",values=np.array([0,0,2],dtype=np.int64))
    ends_3 = gs.Constant("ends_w",values=np.array([1,num_array,3],dtype=np.int64))

    starts_4 = gs.Constant("starts_h",values=np.array([0,0,3],dtype=np.int64))
    ends_4 = gs.Constant("ends_h",values=np.array([1,num_array,4],dtype=np.int64))

    # output
    x = gs.Variable(name="x_center",shape=(1,num_array,1),dtype=np.float32)
    y = gs.Variable(name="y_center",shape=(1,num_array,1),dtype=np.float32)
    w = gs.Variable(name="w",shape=(1,num_array,1),dtype=np.float32)
    h = gs.Variable(name="h",shape=(1,num_array,1),dtype=np.float32)
    ...
    # output
    w_ = gs.Variable(name="w_half_",shape=(1,num_array,1),dtype=np.float32)
    wplus = gs.Variable(name="w_half_plus",shape=(1,num_array,1),dtype=np.float32)
    h_ = gs.Variable(name="h_half_",shape=(1,num_array,1),dtype=np.float32)
    hplus = gs.Variable(name="h_half_plus",shape=(1,num_array,1),dtype=np.float32)
    ...
    #变换2
    # output
    x1 = gs.Variable(name="x1",shape=(1,num_array,1),dtype=np.float32)
    y1 = gs.Variable(name="y1",shape=(1,num_array,1),dtype=np.float32)
    x2 = gs.Variable(name="x2",shape=(1,num_array,1),dtype=np.float32)
    y2 = gs.Variable(name="y2",shape=(1,num_array,1),dtype=np.float32)
    ...
    boxes_0 = gs.Variable(name="boxes_0",shape=(1,num_array,4),dtype=np.float32)
    ...
    shapes = gs.Constant("shape",values=np.array([1,num_array,1,4],dtype=np.int64))

    # output
    boxes = gs.Variable(name="boxes",shape=(1,num_array,1,4),dtype=np.float32)
    ...
    #----处理prob
    scores = gs.Variable(name="scores",shape=(1,num_array, numClasses),dtype=np.float32)
