...
def add_postprocess(input_name, numClasses, output_name):
    ...
    _, _, num_array  = origin_output.outputs[0].shape # yolov8 shape: [batch_size, 4+cls, num_array]
    print(f"num_array: {num_array}")

    output_t = gs.Variable(name="output_t",shape=(1,num_array,8),dtype=np.float32)
    ...
    ends_conf = gs.Constant("ends_conf",values=np.array([1,num_array,numClasses+4],dtype=np.int64))
