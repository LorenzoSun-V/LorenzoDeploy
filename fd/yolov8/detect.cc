...
void InferImage(fastdeploy::vision::detection::YOLOv8& model, const std::string& image_file, const std::string& output_folder);
void InferVideo(fastdeploy::vision::detection::YOLOv8& model, const std::string& video_file, const std::string& output_folder);
...
int main(int argc, char* argv[]) {
  ...
  int img_size = 640;
  if (argc < 4) {
    std::cout << "Usage: ./detect path/to/model path/to/folder run_option [path/to/output_folder]" << std::endl;
    return -1;
  }
  if (argc >= 5) {
