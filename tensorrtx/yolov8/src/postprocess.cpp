...
void draw_bbox(std::vector<cv::Mat>& img_batch, std::vector<std::vector<Detection>>& res_batch) {
  for (size_t i = 0; i < img_batch.size(); i++) {
    ...
    for (size_t j = 0; j < res.size(); j++) {
      ...
      std::string strid = std::to_string((int)res[j].class_id) +':'+ std::to_string(res[j].conf);
      cv::putText(img, strid, cv::Point(r.x, r.y-5), cv::FONT_HERSHEY_COMPLEX, 1, cv::Scalar(0, 255, 255), 1, 8, 0);  
    }
  }
}
